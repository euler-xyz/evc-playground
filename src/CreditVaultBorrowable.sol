...
contract CreditVaultBorrowable is CreditVaultSimple {
    ...
    function disableController(address account) external override nonReentrant {
        if (owed[account] == 0) cvc.disableController(account);
    }
    ...
    nonReentrant {
        ...
        vaultStatusSnapshot();
        address msgSender = CVCAuthenticate(msg.sender, true);
        ...
        if (owed[msgSender] == 0) cvc.disableController(msgSender);
    ...
    nonReentrant {
        ...
        if (owed[receiver] == 0) cvc.disableController(receiver);
    ...
    nonReentrant {
        ...
        require(from != msgSender, "SELF_TRANSFER");
        ...
        if (owed[from] == 0) cvc.disableController(from);
        if (owed[msgSender] == 0) cvc.disableController(msgSender);
